...
function clearMainScreenBridge() {
  if (isVoiceHostWindow()) return;
  void closeAllScreenPopouts();
  clearAllScreenBridge();
}

function teardownMainScreenBridge() {
  if (isVoiceHostWindow()) return;
  void closeAllScreenPopouts();
  teardownScreenBridgeForVoiceLeave();
}

function registerMainScreenTrack(trackSid: string, track: MediaStreamTrack) {
  if (isVoiceHostWindow()) return;
  registerScreenTrack(trackSid, track);
}

function unregisterMainScreenTrack(trackSid: string) {
  if (isVoiceHostWindow()) return;
...
export function useVoiceEngine() {
    ...
    (room: Room) => {
      ...
      room.localParticipant.trackPublications.forEach((pub) => {
        if (
          ...
          if (media) registerMainScreenTrack(pub.trackSid, media);
      ...
      room.remoteParticipants.forEach((p) => {
        ...
        p.trackPublications.forEach((pub) => {
          ...
          if (getUserVoicePref(p.identity).hideVideo) {
            ...
            unregisterMainScreenTrack(pub.trackSid);
            return;
          }
      ...
      setRemoteScreens((prev) => {
        for (const old of prev) {
          if (!seen.has(old.trackSid)) {
            unregisterMainScreenTrack(old.trackSid);
  ...
  const endScreenShare = useCallback(
    async (room: Room | null, stopTracks: boolean) => {
      ...
      for (const pub of pubs) {
        ...
        if (pub.trackSid) unregisterMainScreenTrack(pub.trackSid);
        const track = pub.track;
        if (!track) continue;
        try {
  ...
  const teardownRoom = useCallback(
    async (room: Room, stopTracks: boolean) => {
      await hardTeardownLivekitRoom(room, stopTracks, purgeRemoteAudio);
      requestRendererMemoryTrim();
    },
    [purgeRemoteAudio],
  );
